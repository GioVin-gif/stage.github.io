 ...
 *
 * CORS / FILE:// ORIGIN
 *   Quando l'app è aperta come file:// i browser inviano "null" come origine,
 *   il che fa scattare le CORS policy di quasi tutte le API pubbliche.
 *   La costante IS_FILE_ORIGIN e la funzione proxyFetch() gestiscono
 *   questo caso automaticamente, instradando le chiamate attraverso
 *   corsproxy.io che aggiunge i header CORS necessari.
 *   Quando l'app è servita via http(s):// il proxy viene bypassato.
 *
 * MODALITÀ:
...
/* ═══════════════════════════════════════════════════════════════
   COSTANTI API
═══════════════════════════════════════════════════════════════ */
/* FIX: l'endpoint corretto è /query (POST search), non / (GET list) */
const PATENTSVIEW_URL = 'https://search.patentsview.org/api/v1/patents/query';
const LENS_URL        = 'https://api.lens.org/patent/search';

/* ── CORS PROXY ──────────────────────────────────────────────────
   Quando la pagina è aperta come file:// il browser invia "null" come
   origine; quasi tutte le API rifiutano l'origine null per sicurezza.
   Usiamo una lista di proxy CORS pubblici con fallback automatico.

   IMPORTANTE: localhost e 127.0.0.1 NON hanno bisogno di proxy —
   da un server HTTP locale le chiamate vanno dirette senza blocchi CORS.
   Il proxy serve SOLO per file:// (origine null).
────────────────────────────────────────────────────────────────── */
/* FIX: solo file:// usa proxy; localhost va diretto (risolve CORS nativamente) */
const IS_FILE_ORIGIN = (window.location.protocol === 'file:');

/**
 * Proxy CORS che supportano POST con body JSON e header Authorization.
 * - corsproxy.io       : proxy generico, supporta POST e header custom
 * - thingproxy         : fallback affidabile per POST
 * api.codetabs.com è stato rimosso: blocca le richieste POST.
 */
const CORS_PROXIES = [
 ...
 * - Se l'app non gira da file://, la chiamata va diretta.
 * - Altrimenti prova ogni proxy in sequenza finché uno funziona.
 *
 * @param {string}      url
...
const proxyFetch = async (url, options = {}) => {
    ...
    console.group('[proxyFetch] file:// origin — tentativo con proxy CORS');
    ...
    console.groupEnd();
    throw new Error(
        `CORS bloccato — tutti i proxy hanno fallito. ` +
        `Soluzione: apri una console nella cartella del progetto ed esegui:\n` +
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* ── MOSTRA BADGE PROXY SE FILE:// ────────────────────────── */
    if (IS_FILE_ORIGIN && proxyBadge) {
